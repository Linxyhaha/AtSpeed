...
class StrictKDModel(LlamaForCausalLM):
    def __init__(self, config, KD_ratio=0.5, topK=20):
    ...
    def loss_strict(self, q_logits, p_logits, K, constrained=True):
        ...
        if constrained:
            q_logits, p_logits = q_logits[..., 32000:], p_logits[..., 32000:] # 做一个logits范围的调整，限制在新加的词表上面
        
        q_logits_topk, q_topk_index =torch.topk(q_logits, K)
        p_logits_topk = torch.gather(p_logits, 1, q_topk_index) # (batch_size*seq_len, K)
        
        p_log_probs = F.log_softmax(p_logits_topk, dim=-1) # (batch_size*seq_len, K)
        q_probs = F.softmax(q_logits_topk, dim=-1) # (batch_size*seq_len, K)
        p_probs_k = torch.exp(p_log_probs[...,-1]) # the k-th prob of the teacher model, (batch_size*seq_len, 1)

        loss_rKLD = self.loss_fct_KLD(p_log_probs, q_probs)
        loss_reg = -torch.mean(p_probs_k * torch.sum((q_probs * torch.log(q_probs)), dim=-1))
...
class SoftKDModel(LlamaForCausalLM):
    def __init__(self, config, KD_ratio=0.5, topK=20, len_y=5, use_cache=True, debug=False, allowed_tokens_dict=None, constrained_softmax=False, tau=1.0, weights=None, topK4loss=0, topKto1=False, topK_p_normal=False):
        super().__init__(config)
        self.topK = topK
        self.len_y = len_y
        self.KD_ratio = KD_ratio
        ...
        self.topK4loss = topK4loss
        self.topKto1 = topKto1
        self.topK_p_normal = topK_p_normal
    ...
    def loss_soft(self, q_logits, p_logits, constrained=True):
        ...
        q_logits = q_logits / self.tau  ###
        p_logits = p_logits / self.tau  ###
        ...
        if self.topK4loss > 0:
            p_probs = p_probs[:, :4, :]  ###
            q_probs = q_probs[:, :4, :]  ###
            p_probs_topk, p_topk_index = torch.topk(p_probs, self.topK4loss, dim=-1)
            q_probs_topk = torch.gather(q_probs, -1, p_topk_index)
            p_probs = p_probs_topk
            q_probs = q_probs_topk
            if self.topK_p_normal:
                p_probs = p_probs / p_probs.sum(dim=-1, keepdim=True)
            if self.topKto1:
                p_probs = 1

        if self.weights is not None:
            weights = torch.tensor(self.weights).to(q_probs.device)
            weights = weights / weights.sum()
            weights = weights.expand(q_probs.shape[0], q_probs.shape[1])
            loss_TVD = ((0.5 * torch.abs(q_probs - p_probs)).sum(dim=-1) * weights).sum(dim=-1).mean()
        else:
            loss_TVD = (0.5 * torch.abs(q_probs - p_probs)).sum(dim=-1).mean()
        # print('--- line 190 --- loss_TVD:', loss_TVD)
        return loss_TVD
    ...
    def forward(
        ...
        # outputs.past_key_values 是个二元数组，其中每个entry是一个二元数组，每一个entry的shape都是 # (batch_size, num_head, seq_len, dim/num_head)
        # 最里层，一个k, 一个v，shape一样，外层是第一层的kv和第二层的kv

        # 1. 拿着teacher output输入student model得到student pred (这里需要复用kvcache不然很慢, 把input和student output拼在一起，复用input的kv-cache)- ok
        # 2. 利用新的prediction和teacher output送进soft_loss - ok
        # 3. 写完后测试一下use_cache前后的结果是否一致 - ok
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if self.KD_ratio != 0:
                ...
                # print('--- line 190 --- loss_KLD:', loss_KLD)
                loss = (1 - self.KD_ratio) * loss + self.KD_ratio * loss_KLD
            ### End of added lines
...
class OnPolicyStrictKDModel(LlamaForCausalLM):
    def __init__(self, config, teacher_model=None, prefix_allowed_tokens=None, KD_ratio=0.5, topK=20, beta=1.0, r_original=1.0, r_student=0.0, generate_batchsize=4, teacher_batchsize=4, len_y=4, use_cache=True, world_size=2, debug=False, allowed_tokens_dict=None, constrained_softmax=False, tau=1.0):
        super().__init__(config)
        self.KD_ratio = KD_ratio
    ...
    def loss_strict(self, q_logits, p_logits, K, constrained=True):
        ...
        # q_log_probs_topk = q_log_probs_topk / self.tau  ###
        # p_log_probs_topk = p_log_probs_topk / self.tau  ###
        if self.constrained_softmax:
            p_log_probs = F.log_softmax(p_log_probs_topk, dim=-1)
            q_log_probs = F.log_softmax(q_log_probs_topk, dim=-1)
        else:
    ...
    def get_teacher_logits(self, data):
        ...
        for idx, batch_data in tqdm(enumerate(batch(data))):
            ...
            if int(os.environ.get("LOCAL_RANK") or 0) == 0:
                ipdb.set_trace()
            dist.barrier() # 检查data的每个item是不是都是tensor，都对应sequence input长度
    ...
    def select_data(self, data, past_key_values=None):
        ...
        if u1 < self.original_ratio:
            ...
            if u2 < self.student_ratio:
                y_student = self.student_generate(self.generate_batchsize, self.topK, past_key_values, **data) # 生成学生output的函数，return回来student的beam output
                ...
                sample = self.combine_x_y(data, y_student)
                sample_logits = self.get_teacher_logits(sample) # 拿到teacher logits的函数，return回来teacher的每个位置的logits
    ...
    def forward(
        ...
        # 判断，如果original，直接进入3,和4
        # 如果teacher output, 用kv-cache，得到student logits Yp, 和传入的teacher logits进入3，4
        # 如果student ouput, 用kv-cache，得到student logits Yp, 和之前已经计算并返回的teacher logits，进入3，4
        ...
        if source == "original":
            ...
            # TODO: kv-cache 拿着original prefix 的kvcache和teacher的output做
            if self.use_cache:
                # if self.debug:
                #     print("Now is DEBUG mode!!!")
                #     teacher_output = torch.ones((input_ids.shape[0], self.topK, self.len_y), dtype=torch.int64).cuda() # test ONLY!!!
            ...
            # TODO: kv-cache 拿着original prefix 的kvcache和student的output做
            if self.use_cache:
                if self.debug:
                    print("Now is DEBUG mode!!!")
                ...
                student_Yq_logits = torch.cat([logits[:,-self.len_y-1,:].unsqueeze(1).repeat(1,self.topK,1).view(-1,1,logits.shape[-1]), student_Yq_logits[:,-self.len_y:-1,:]], dim=1) # 前移一位拿logits
            ...
            if labels is not None:
                ...
                teacher_Yp_logits = teacher_Yp_logits.view(-1, student_Yp_logits.shape[-2], student_Yp_logits.shape[-1])
                teacher_Yq_logits = teacher_Yq_logits.to(student_Yq_logits.device) # 存的是 -len_y-1:-1
...
class OnPolicySoftKDModel(LlamaForCausalLM):
    def __init__(self, config, KD_ratio=0.5, topK=20):
        super().__init__(config)
        self.topK = topK
        self.KD_ratio = KD_ratio
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        teacher_logits: Optional[torch.FloatTensor] = None,
        teacher_output: Optional[torch.FloatTensor] = None,
        teacher_output_logits: Optional[torch.FloatTensor] = None,
    ) -> Union[Tuple, CausalLMOutputWithPast]:
        ...
        # TODO: 类比seqKD的写法复用input, 拼上新的model-generated output做一次forward
        # 1. 先拿到teacher对应当前input的 beam output 
        # 2. 复用前面的kv cache等，拼上新的output，拿到prediction
        # 3. 利用新的prediction和teacher output送进soft_loss
        ...
        if labels is not None:
            ...
            ### Added lines
            vocab_size = teacher_logits.shape[-1]
            student_logits = logits[:, -5:-1].contiguous().view(-1, vocab_size)
            teacher_logits = teacher_logits.view(-1, vocab_size)
            loss_KLD = self.loss_soft(student_logits, teacher_logits, self.topK)
            loss = (1 - self.KD_ratio) * loss + self.KD_ratio * loss_KLD
            ### End of added lines
...
class SeqKDModel_old(LlamaForCausalLM):
    def __init__(self, config, KD_ratio=0.5, teacher_beam_size=1):
        super().__init__(config)
        self.KD_ratio = KD_ratio
        self.teacher_beam_size = teacher_beam_size
        self.loss_CE_sum = CrossEntropyLoss(reduction="sum")
    ...
    def forward(
        ...
        if labels is not None:
            ### Modified lines
            vocab_size = teacher_logits.shape[-1]
            n_pred = teacher_logits.shape[1]
            n_prefix = logits.shape[1] - 1 - n_pred

            logits_prefix = logits[:, :n_prefix, :].contiguous()
            labels_prefix = labels[:, 1:-n_pred].contiguous()
            loss_prefix = self.loss_CE_sum(logits_prefix.view(-1, vocab_size), labels_prefix.view(-1))

            logits_pred = logits[:, n_prefix:-1, :].contiguous()
            labels_pred = labels[:, -n_pred:].contiguous()
            n_batch_prefix = (labels_prefix >= 0).sum()
            n_batch_pred = (labels_pred >= 0).sum()
            
            # original loss
            loss_pred = self.loss_CE_sum(logits_pred.view(-1, vocab_size), labels_pred.view(-1))
            loss_original = (loss_prefix + loss_pred) / (n_batch_prefix + n_batch_pred)

            # seqKD loss
            teacher_scores = F.log_softmax(teacher_logits, dim=-1) # (batch_size, n_pred, vocab_size)
            beam_sequence = input_ids[:, :-n_pred].unsqueeze(1)
            beam_socres = torch.zeros(1, device=teacher_scores.device)
            for i in range(n_pred):
                scores = teacher_scores[:, i, :] # (batch_size, vocab_size)
                #print('-----------------')
                '''
                print('scores: ', scores.shape)
                print('beam_socres: ', beam_socres.shape)
                print('beam_scores[:, None]: ', beam_socres[:, None].shape)
                print('beam_scores[:, None].expand_as(scores): ', beam_socres[:, None].expand_as(scores).shape)
                '''
                beam_scores = scores + beam_socres[:, None].expand_as(scores)
                beam_scores, next_tokens = beam_scores.topk(self.teacher_beam_size, dim=-1)
                ...
                beam_prefix = torch.gather(beam_sequence, 1, beam_idx.unsqueeze(-1).expand(-1, -1, beam_sequence.shape[-1]))
                '''
                print('beam_idx: ', beam_idx.shape)
                print('beam_tokens: ', beam_tokens.shape)
                print('beam_prefix: ', beam_prefix.shape)
                print('beam_sequence: ', beam_sequence.shape)
                '''
                beam_sequence = torch.cat((beam_prefix, beam_tokens.unsqueeze(-1)), dim=-1)
            '''
                print('beam_sequence_cat: ', beam_sequence.shape)
            print('beam_sequence: ', beam_sequence.shape)
            ...
            '''
            beam_logits = logits_pred.repeat_interleave(self.teacher_beam_size, dim=0)
            beam_labels = beam_sequence[..., -n_pred:].contiguous()
            loss_seqKD_pred = self.loss_CE_sum(beam_logits.view(-1, vocab_size), beam_labels.view(-1))
            loss_seqKD = (self.teacher_beam_size * loss_prefix + loss_seqKD_pred) / (self.teacher_beam_size * (n_batch_prefix + n_batch_pred))

            loss = (1 - self.KD_ratio) * loss_original + self.KD_ratio * loss_seqKD
